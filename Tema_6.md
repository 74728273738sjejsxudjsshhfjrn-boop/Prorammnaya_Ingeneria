...
###Вывод: Данная программа на вход получает номер кабинета, а выводит необходимую информацию
...
###Вывод: Данная программа принимает неограниченное количество параметров «ключ: значение» и обновляет созданный им словарь
...
###Вывод: Данная программа раскладывает строку на отдельные символы
...
###Вывод: Данная программа будет писать имя, возраст и место работы, но при этом на вход этой функции будет поступать кортеж
...
###Вывод: Данная программа сортирует кортеж, состоящий из целых чисел по возрастанию, и возвращает его
...
###Вывод: Данная программа принимает последовательность чисел, разделенных пробелом, а после форматирует эти данные в список и кортеж
...
###Вывод: Данная программа удаляет первое появление определенного элемента из кортежа по значению и возвращает кортеж без него
...
###Вывод: Данная программа выполняет функции: вернуть новый кортеж, начинающийся с первого появления элемента в нем и заканчивающийся вторым его появлением включительно. Если элемента нет вовсе вернуть пустой кортеж. Если элемент встречается только один раз, то вернуть кортеж, который начинается с него и идет до конца исходного.
...
###Вывод: Программа принимает список чисел и возвращает кортеж с уникальными элементами этого списка. Также программа выводит количество уникальных элементов.


#Общий вывод по теме
В данной работе были изуечны словари и кортежи, а так же их основныве отличия друг от друга. Кортежи имеют достаточное преимущество над словарями, однако ни в коем случае они не являются улучшенной заменой. Обе функции языка Python очень важны и необходимы для качественной разработки.
