...
###Вывод: Данная программа корректно обрабатывает ввод имен и выводит сообщения в зависимости от того, угадано ли имя.
...
###Вывод: В данной программе с помощью функции реализуется определение наличия или отсутсвия топпинга в мороженом
...
###Вывод: Данная программа показывает как работает класс не просто инкапсуляцией, а классом с сеттером, геттером и деструктором.
Метод класса, который служит конструктором, должен называться init. Здесь он назван просто init, что является ошибкой. Опечатка в методе get_value. В данном методе вместо _value должно быть value, так как именно это имя используется в сигнатуре свойства.
...
###Вывод: Данная программа показывает работу "наследования" классов
...
###Вывод: Данная программа помогаяет наглядно увидеть, как работает декоратор @staticmethod,также показан пример полиморфизма.
...
3.Создайте метод init(), внутри которого будут определены два динамических свойства:_index (передается параметром) и_state (принимает первое значение из словаря states). После написания этого блока кода в комментарии к нему укажите какими являются эти два свойства
...
###Вывод: В этом задании я научилась создавать и связывать классы в Python, применять инкапсуляцию, наследование и полиморфизм для моделирования процессов созревания и сбора урожая.


### Общий вывод: В ходе работы я изучила основные концепции объектно-ориентированного программирования (ООП): инкапсуляцию, наследование и полиморфизм. Освоила создание классов и методов в Python, работу с конструкторами, сеттерами и геттерами, а также применение декоратора @staticmethod. Научилась практическому использованию классов и объектов, а также организации кода для взаимодействия между ними, что помогает эффективно структурировать программы и упрощает их дальнейшее сопровождение.
